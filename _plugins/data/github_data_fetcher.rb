...
module Jekyll
  class GitHubDataFetcher < Generator
    ...
    def generate(site)
      ...
      if token
        ...
        begin
          ...
          use_existing_data(site, data_file, username)
        end
      else
    ...
    def use_existing_data(site, data_file, username)
      if File.exist?(data_file)
        Jekyll.logger.info 'GitHub Data Fetcher:', 'Using existing github_stats.yml'
        begin
          existing = YAML.safe_load(File.read(data_file)) || {}
          site.data['github_stats'] = existing
        rescue StandardError => e
          Jekyll.logger.warn 'GitHub Data Fetcher:', "Error reading existing data: #{e.message}"
        end
      else
        ...
        site.data['github_stats'] = placeholder
